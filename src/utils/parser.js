...
function extractPhones(text) {
	const re =
		/(?:\+?\d{1,4}[\s.-]?)?(?:\(?\d{2,5}\)?[\s.-]?)?(?:\d[\s.-]?){5,15}\d/g;
	const matches = text.match(re) || [];

	// Filter out short matches (< 7 digits) that are likely not phone numbers
	// and extract only numerical characters
	return matches.map((m) => m.replace(/\D/g, "")).filter((m) => m.length >= 7);
}

/**
 * Pick the best mobile phone number from candidates.
 * Prefers mobile-style numbers (starts with 08, +62 8, etc.) over landlines.
 */
function pickMobileNumber(phones) {
	if (phones.length === 0) return "";

	// Mobile indicators: starts with 08, or country code + mobile prefix (e.g. +628)
	const mobilePattern = /^(?:\+?62|0)8/;
	const mobile = phones.find((p) => mobilePattern.test(p));
...
function extractName(lines, knownValues) {
	...
	for (const line of lines) {
		...
		const alphaRatio =
			originalTrimmed.replace(/[^a-zA-Z\s]/g, "").length /
			originalTrimmed.length;
		if (alphaRatio < 0.75) continue;

		// Remove garbage characters at ends
		const trimmed = originalTrimmed
			.replace(/^[^a-zA-Z]+/, "")
			.replace(/[^a-zA-Z]+$/, "");
		...
		if (knownValues.some((v) => v && lowerTrimmed.includes(v.toLowerCase())))
			continue;
		...
		if (
			titleCasedWords.length >= 2 ||
			(words.length === 2 &&
				titleCasedWords.length === 1 &&
				/^[A-Z]/.test(words[0]))
		) {
...
export function parseContactText(rawText) {
	...
	const mobile = pickMobileNumber(phones);
