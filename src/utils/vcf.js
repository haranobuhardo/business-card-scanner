 ...
 * @param {object} contact - { name, email, mobile, company, website }
 * @returns {string} VCard formatted string
 */
export function generateVCard(contact) {
	const { name, email, mobile, company, website } = contact;
...
export function getWhatsAppUrl(contact, template = "Hello {name}!") {
	...
	const cleanPhone = contact.mobile.replace(/[^\d+]/g, "").replace(/^\+/, "");
